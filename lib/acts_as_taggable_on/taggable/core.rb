  ...
  module Core
    ...
    def process_dirty_object(context, new_list)
      ...
      if changed_attributes.include?(attrib)
        ...
        send("#{attrib}=", []) if old.to_s == value.to_s
      else
        old = tag_list_on(context)
        if self.class.preserve_tag_order
          send("#{attrib}=", old) if old.to_s != value.to_s
        else
          if old.sort != ActsAsTaggableOn.default_parser.new(value).parse.sort
            send("#{attrib}=", old.to_s)
          end
        end
      end
    end
